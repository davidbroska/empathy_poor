...
composite_mes <- c("empa_conc", "persp_tak", "me_dispos_attr", "me_situational_attr", "sub_effort_1")
composite_dvs <- c("GenWelfSupp", "Welf4Poor", "Welf4HardWork", 
                   "SpecWelf4Mobility", "SpecWelf4Poor","IneqMagnPercep")
...
df_GenWelfSupp <- df %>% select(dv_gen_welfare_1, dv_gen_welfare_2rec) 
df_Welf4Poor <- df %>% select(dv_welfare_poor_hard_1, dv_welfare_poor_hard_2) 
df_Welf4HardWork <- df %>% select(dv_welfare_poor_hard_13, dv_welfare_poor_hard_14)
df_SpecWelf4Mobility <- df %>% select(dv_mobility_pol_10, dv_mobility_pol_2,
                                         dv_mobility_pol_1, dv_mobility_pol_3) 
df_SpecWelf4Poor <- df %>% select(dv_spec_welfare_pol_1, dv_spec_welfare_pol_2,
                                       dv_spec_welfare_pol_13, dv_spec_welfare_pol_14)
df_IneqMagnPercep <- df %>% select(dv_ineq_1)

# create composite items 
df$GenWelfSupp <- df_GenWelfSupp %>% {{rowSums(.) / ncol(.)}}
df$Welf4Poor <- df_Welf4Poor %>% {{rowSums(.) / ncol(.)}}
df$Welf4HardWork <- df_Welf4HardWork %>% {{rowSums(.) / ncol(.)}}
df$SpecWelf4Mobility <- df_SpecWelf4Mobility %>% {{rowSums(.) / ncol(.)}}
df$SpecWelf4Poor <- df_SpecWelf4Poor %>% {{rowSums(.) / ncol(.)}}
df$IneqMagnPercep <-  df %>% select(dv_ineq_1) %>% pull()
...
tab_composite_dv_key <- tibble(composite = composite_dvs, 
       variables = c(paste0(colnames(df_GenWelfSupp), collapse=", "),
                     paste0(colnames(df_Welf4Poor), collapse=", "), 
                     paste0(colnames(df_Welf4HardWork), collapse=", "),
                     paste0(colnames(df_SpecWelf4Mobility), collapse = ", "),
                     paste0(colnames(df_SpecWelf4Poor), collapse = ", "),
                     paste0(colnames(df_IneqMagnPercep), collapse=", "))) %>% 
...
tab_composite_me_key <- tibble(composite = composite_mes, 
       variables = c(paste0(colnames(all_empathy_df), collapse=", "),
                     ...
                     "sub_effort_1")) %>% 
  mutate(label  = composite %>%  str_replace_all("empa_conc", "Empathy") %>%
           str_replace_all("persp_tak", "PerspTak") %>%
           str_replace_all("me_dispos_attr", "DispAttr") %>% 
           str_replace_all("me_situational_attr", "SitAttr") %>%
           str_replace_all("sub_effort_1", "SubjMob") %>%
           factor(levels = c("Empathy", "PerspTak", "DispAttr", "SitAttr","SubjMob"))) %>% 
...
# cronbach's alpha
a_dv_GenWelfSupp <- psych::alpha(df_GenWelfSupp)
a_dv_Welf4Poor <- psych::alpha(df_Welf4Poor)
a_dv_Welf4HardWork <- psych::alpha(df_Welf4HardWork)
a_dv_SpecWelf4Mobility <- psych::alpha(df_SpecWelf4Mobility)
a_dv_SpecWelf4Poor <- psych::alpha(df_SpecWelf4Poor)
a_dv_IneqMagnPercep <- NA



dv_raw_alphas = list(a_dv_GenWelfSupp, a_dv_Welf4Poor, a_dv_Welf4HardWork, a_dv_SpecWelf4Mobility, a_dv_SpecWelf4Poor,a_dv_IneqMagnPercep) %>% lapply(function(l) l[[1]]) %>% do.call(rbind, .)
...
```{r cronbach-alpha-me}
a_SubjMob<-NA
me_raw_alphas = list(a_empathy, a_persp_tak, a_dispositional, a_situational, a_SubjMob) %>% lapply(function(l) l[[1]]) %>% do.call(rbind, .)
  
tibble(var = composite_mes, me_raw_alphas[,1:3])  %>% 
  mutate(var = str_replace_all(var, "empa_conc", "Empathy") %>% 
                           str_replace_all("persp_tak", "PerspTak") %>% 
                           str_replace_all("me_situational_attr","SitAttr")%>%
                           str_replace_all("me_dispos_attr", "DispAttr") %>% 
                           str_replace_all("sub_effort_1", "SubjMob") %>% 
...
plot_dv <- function(dependent_var, ylab="Agreement", xlab="Empathy Condition",caption = "", subtitle=NA){
  ...
  ggplot(dd, aes(empathy_condition, y)) + 
    ...
    geom_text(aes(x=empathy_condition,y=-3, 
                  label=paste0("m=",round(mean.value,1),"\nse=",round(se.mean,1))), size=2, fontface="plain")
...
```{r, fig.cap="Number of respondents who experienced upward mobility, downward mobility, or no change in socioeconomic status", out.width="65%"}
...
```{r composite-GenWelfSupp-Welf4Poor, fig.cap="GenWelfSupp and Welf4Poor composite"}
composite_dv_plots_pairs[[1]]
```

```{r composite-Welf4HardWork-SpecWelf4Mobility, fig.cap="Welf4HardWork and SpecWelf4Mobility"}
composite_dv_plots_pairs[[2]]
```

```{r composite-SpecWelf4Poor-IneqMagnPercep, fig.cap="SpecWelf4Poor and IneqMagnPercept composite"}
...
```{r composite-empa-persp, fig.cap="Empathetic concern and perspective taking composite"}
composite_me_plots_pairs[[1]]
```

```{r composite-attribution, fig.cap="Dispositional and situational attribution composite"}
...
sm_summs$dv <- lapply(names(sm), function(n) rep(n, length(coef(sm[[1]])))) %>% unlist()
sm_summs <- sm_summs %>% 
...
# create column with dv name 
dm_summs$dv <- lapply(names(dm), function(n) rep(n, length(coef(dm[[1]])))) %>% unlist()
# dm_summs <- dm_summs %>% 
#   mutate(term = factor(term, levels = c("(Intercept)", "mobility_conditionlow", "empathy_conditiontreatment", "mobility_conditionlow:empathy_conditiontreatment"), 
#                 labels = c("(Intercept)", "Low mobility", "Homelessness", "Low mob. x Homelessness")), 
...
sm_summs %>% 
  ...
  mutate(dv = str_remove(dv, "sm_") %>% factor(levels = composite_dvs[length(composite_dvs):1])) %>% 
...
dm_summs %>% 
  ...
  mutate(dv=dv %>% str_remove("dm_") %>% factor(levels=composite_dvs[length(composite_dvs):1])) %>% 
  ggplot(aes(estimate, dv, height=0, xmin=conf.low,xmax=conf.high)) +  
  geom_point() + 
  facet_wrap(~ term) +
  geom_vline(xintercept = 0, lty = 4) + 
...
# create column with dv name 
sm_me_summs$dv <- lapply(names(sm_me), function(n) rep(n, length(coef(sm_me[[1]])))) %>% unlist()
...
sm_me_names <- data.frame(dv = names(sm_me)) %>% 
  mutate(label = dv %>% str_remove("sm_") %>% 
           str_replace_all("empa_conc", "Empathy") %>% 
           str_replace_all("persp_tak", "PerspTak") %>%
           str_replace_all("me_situational_attr","SitAttr")%>%
           str_replace_all("me_dispos_attr", "DispAttr") %>%
           str_replace_all("sub_effort_1", "SubjMob") %>%
           factor(levels = c("SitAttr", "DispAttr", "PerspTak","Empathy","SubjMob")))
```
...
sm_me_noI_names <- data.frame(dv = names(sm_me_noI)) %>% 
  mutate(label = dv %>% str_remove("sm_me_noI_") %>% 
           ...
           str_replace_all("me_dispos_attr", "DispAttr") %>% 
           str_replace_all("sub_effort_1", "SubjMob") %>% 
             factor(levels = c("SubjMob", "SitAttr", "DispAttr", "PerspTak","Empathy")))
```
...
p_low_mob_effects <- mm_pck %>% 
  ...
  geom_point(size=1.2) + 
  geom_vline(xintercept = 0, lty = 4) +
  geom_text(aes(x=26,y=dv,label=estimate), 
  ...
  facet_grid(~ mediator, 
             labeller = as_labeller(c("empa_conc"="Empathetic\nconcern",
                                      "persp_tak"="Perspective\ntaking", "me_dispos_attr"="Dispositional\nattribution", "sub_effort_1"="Subjective\nMobility", "me_situational_attr"="Situational\nattribution"))
             ) + 
...
p_high_mob_effects <- mm_pck %>% 
  ...
  geom_point(size=1.2) + 
  geom_vline(xintercept = 0, lty = 4) +
  geom_text(aes(x=16,y=dv,label=estimate), 
...
mo_hi <- df %>% 
  mutate(mobility_condition = factor(mobility_condition, levels = c("high","low"))) %>% 
  lm(GenWelfSupp ~ empathy_condition * mobility_condition, .)
...
dd <- data.frame(yhat = predict(sm$sm_GenWelfSupp),
                 poverty = sm$sm_GenWelfSupp$model$empathy_condition, 
                 mobility = sm$sm_GenWelfSupp$model$mobility_condition)
...
```{r}
ggplot(df, aes(income_num, GenWelfSupp, color=empathy_condition)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  facet_grid(~ mobility_condition, 
             labeller = as_labeller(c("high"="High mobility","low"="Low mobility"))) +
  theme(legend.position = "bottom") + 
  labs(color = "Empathy condition")
```
...
1. __empathetic concern__ in the low mobility but not in the high mobility condition. In the low mobility condition, empathy explains at least 38% and at most 73% of the effect of poverty exposure on welfare policy support. 
...
3. __dispositional attributions__ in the low mobility condition (from 23% to 38% explained) but exposure to poverty was only marginally mediated in the high mobility condition (from 4% to 8% explained).
4. __situational attributions__ in the low mobility condition (from 34% to 58% explained) and even stronger in high mobility condition (from 34% to 76% explained). 
